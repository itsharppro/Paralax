...
echo "Starting build and NuGet package creation for Paralax.Docs.Swagger..."

cd src/Paralax.Docs.Swagger/src/Paralax.Docs.Swagger

echo "Restoring NuGet packages..."
dotnet restore

PACKAGE_VERSION="1.0.$GITHUB_RUN_NUMBER"
echo "Building and packing the Paralax.Docs.Swagger library..."
dotnet pack -c release /p:PackageVersion=$PACKAGE_VERSION --no-restore -o ./nupkg

PACKAGE_PATH="./nupkg/Paralax.Docs.Swagger.$PACKAGE_VERSION.nupkg"
...
if [ -f "$PACKAGE_PATH" ]; then
  ...
  echo "Uploading Paralax.Docs.Swagger package to NuGet..."
