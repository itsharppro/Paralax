...
cd src/Paralax.Tracing.Jaeger.RabbitMQ/src/Paralax.Tracing.Jaeger.RabbitMQ

echo "Restoring NuGet packages..."
dotnet restore

PACKAGE_VERSION="1.0.$GITHUB_RUN_NUMBER"
echo "Building and packing the Paralax.Tracing.Jaeger.RabbitMQ library..."
dotnet pack -c release /p:PackageVersion=$PACKAGE_VERSION --no-restore -o ./nupkg

PACKAGE_PATH="./nupkg/Paralax.Tracing.Jaeger.RabbitMQ.$PACKAGE_VERSION.nupkg"
...
if [ -f "$PACKAGE_PATH" ]; then
  ...
  echo "Uploading Paralax.Tracing.Jaeger.RabbitMQ package to NuGet..."
