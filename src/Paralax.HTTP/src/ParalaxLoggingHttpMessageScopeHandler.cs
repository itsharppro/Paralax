...
{
    internal sealed class ParalaxLoggingHttpMessageScopeHandler : DelegatingHandler
    {
        private readonly ILogger _logger;
        private readonly HashSet<string> _maskedRequestUrlParts;
        private readonly string _maskTemplate;

        public ParalaxLoggingHttpMessageScopeHandler(ILogger logger, HttpClientOptions options)
        {
            _logger = logger ?? throw new ArgumentNullException(nameof(logger));
            _ = options ?? throw new ArgumentNullException(nameof(options));
            _maskedRequestUrlParts = new HashSet<string>(options.RequestMasking?.UrlParts ?? Enumerable.Empty<string>());
        ...
        private static class Log
        {
            ...
            public static IDisposable BeginRequestPipelineScope(ILogger logger, HttpRequestMessage request, ISet<string> maskedRequestUrlParts, string maskTemplate)
            {
            ...
            public static void RequestPipelineStart(ILogger logger, HttpRequestMessage request, ISet<string> maskedRequestUrlParts, string maskTemplate)
            {
                var uri = MaskUri(request.RequestUri, maskedRequestUrlParts, maskTemplate);
