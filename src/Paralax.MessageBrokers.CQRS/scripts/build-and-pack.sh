...
cd src/Paralax.MessageBrokers.CQRS/src/Paralax.MessageBrokers.CQRS

echo "Restoring NuGet packages..."
dotnet restore

PACKAGE_VERSION="1.0.$GITHUB_RUN_NUMBER"
echo "Building and packing the Paralax.MessageBrokers.CQRS library..."
dotnet pack -c release /p:PackageVersion=$PACKAGE_VERSION --no-restore -o ./nupkg

PACKAGE_PATH="./nupkg/Paralax.MessageBrokers.CQRS.$PACKAGE_VERSION.nupkg"
...
if [ -f "$PACKAGE_PATH" ]; then
  ...
  echo "Uploading Paralax.MessageBrokers.CQRS package to NuGet..."
