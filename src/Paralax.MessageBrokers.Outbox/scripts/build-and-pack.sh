...
echo "Starting build and NuGet package creation for Paralax.MessageBrokers.Outbox..."

cd src/Paralax.MessageBrokers.Outbox/src/Paralax.MessageBrokers.Outbox

echo "Restoring NuGet packages..."
dotnet restore

PACKAGE_VERSION="1.0.$GITHUB_RUN_NUMBER"
echo "Building and packing the Paralax.MessageBrokers.Outbox library..."
dotnet pack -c release /p:PackageVersion=$PACKAGE_VERSION --no-restore -o ./nupkg

PACKAGE_PATH="./nupkg/Paralax.MessageBrokers.Outbox.$PACKAGE_VERSION.nupkg"
...
if [ -f "$PACKAGE_PATH" ]; then
  ...
  echo "Uploading Paralax.MessageBrokers.Outbox package to NuGet..."
