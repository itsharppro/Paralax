...
cd src/Paralax.Metrics.Prometheus/src/Paralax.Metrics.Prometheus

echo "Restoring NuGet packages..."
dotnet restore

PACKAGE_VERSION="1.0.$GITHUB_RUN_NUMBER"
echo "Building and packing the Paralax.Metrics.Prometheus library..."
dotnet pack -c release /p:PackageVersion=$PACKAGE_VERSION --no-restore -o ./nupkg

PACKAGE_PATH="./nupkg/Paralax.Metrics.Prometheus.$PACKAGE_VERSION.nupkg"
...
if [ -f "$PACKAGE_PATH" ]; then
  ...
  echo "Uploading Paralax.Metrics.Prometheus package to NuGet..."
