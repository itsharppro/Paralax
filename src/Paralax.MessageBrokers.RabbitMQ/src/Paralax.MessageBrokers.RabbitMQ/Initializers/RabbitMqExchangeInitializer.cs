...
namespace Paralax.MessageBrokers.RabbitMQ.Initializers
{
    public class RabbitMqExchangeInitializer : IInitializer
    {
        ...
        public Task InitializeAsync()
        {
            ...
            if (_options.Exchange?.Declare == true)
            {
                ...
                if (_options.DeadLetter?.Enabled == true && _options.DeadLetter.Declare == true)
                {
                    var deadLetterExchange = $"{_options.DeadLetter.Prefix}{_options.Exchange.Name}{_options.DeadLetter.Suffix}";
                    channel.ExchangeDeclare(deadLetterExchange, ExchangeType.Direct, _options.Exchange.Durable,
                        _options.Exchange.AutoDelete);
                }
            }
