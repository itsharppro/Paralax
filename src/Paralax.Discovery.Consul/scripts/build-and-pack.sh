...
echo "Starting build and NuGet package creation for Paralax.Discovery.Consul..."
...
echo "Building and packing the Paralax.Discovery.Consul library..."
dotnet pack -c release /p:PackageVersion=$PACKAGE_VERSION --no-restore -o ./nupkg

PACKAGE_PATH="./nupkg/Paralax.Discovery.Consul.$PACKAGE_VERSION.nupkg"
...
if [ -f "$PACKAGE_PATH" ]; then
  ...
  echo "Uploading Paralax.Discovery.Consul package to NuGet..."
