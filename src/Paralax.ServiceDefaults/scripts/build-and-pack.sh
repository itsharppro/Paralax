...
echo "Starting build and NuGet package creation for Paralax.ServiceDefaults framework..."

cd src/Paralax.ServiceDefaults/src/Paralax.ServiceDefaults

echo "Restoring NuGet packages..."
dotnet restore

PACKAGE_VERSION="1.0.$GITHUB_RUN_NUMBER"
echo "Building and packing the Paralax.ServiceDefaults library..."
dotnet pack -c release /p:PackageVersion=$PACKAGE_VERSION --no-restore -o ./nupkg

PACKAGE_PATH="./nupkg/Paralax.ServiceDefaults.$PACKAGE_VERSION.nupkg"
...
if [ -f "$PACKAGE_PATH" ]; then
  ...
  echo "Uploading Paralax.ServiceDefaults package to NuGet..."
