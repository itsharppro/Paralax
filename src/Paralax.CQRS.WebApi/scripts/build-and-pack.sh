...
echo "Starting build and NuGet package creation for Paralax.CQRS.WebApi..."
...
echo "Building and packing the Paralax.CQRS.WebApi library..."
dotnet pack -c release /p:PackageVersion=$PACKAGE_VERSION --no-restore -o ./nupkg

PACKAGE_PATH="./nupkg/Paralax.CQRS.WebApi.$PACKAGE_VERSION.nupkg"
...
if [ -f "$PACKAGE_PATH" ]; then
  ...
  echo "Uploading Paralax.CQRS.WebApi package to NuGet..."
