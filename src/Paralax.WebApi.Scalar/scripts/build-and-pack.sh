...
cd src/Paralax.WebApi.Scalar/src/Paralax.WebApi.Scalar

echo "Restoring NuGet packages..."
dotnet restore

PACKAGE_VERSION="1.0.$GITHUB_RUN_NUMBER"
echo "Building and packing the Paralax.WebApi.Scalar library..."
dotnet pack -c release /p:PackageVersion=$PACKAGE_VERSION --no-restore -o ./nupkg

PACKAGE_PATH="./nupkg/Paralax.WebApi.Scalar.$PACKAGE_VERSION.nupkg"
...
if [ -f "$PACKAGE_PATH" ]; then
  ...
  echo "Uploading Paralax.WebApi.Scalar package to NuGet..."
