...
cd src/Paralax.LoadBalancing.Fabio/src/Paralax.LoadBalancing.Fabio

echo "Restoring NuGet packages..."
dotnet restore

PACKAGE_VERSION="1.0.$GITHUB_RUN_NUMBER"
echo "Building and packing the Paralax.LoadBalancing.Fabio library..."
dotnet pack -c release /p:PackageVersion=$PACKAGE_VERSION --no-restore -o ./nupkg

PACKAGE_PATH="./nupkg/Paralax.LoadBalancing.Fabio.$PACKAGE_VERSION.nupkg"
...
if [ -f "$PACKAGE_PATH" ]; then
  ...
  echo "Uploading Paralax.LoadBalancing.Fabio package to NuGet..."
