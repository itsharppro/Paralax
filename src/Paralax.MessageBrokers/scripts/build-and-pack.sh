...
echo "Starting build and NuGet package creation for Paralax.MessageBrokers..."
...
echo "Building and packing the Paralax.MessageBrokers library..."
dotnet pack -c release /p:PackageVersion=$PACKAGE_VERSION --no-restore -o ./nupkg

PACKAGE_PATH="./nupkg/Paralax.MessageBrokers.$PACKAGE_VERSION.nupkg"
...
if [ -f "$PACKAGE_PATH" ]; then
  ...
  echo "Uploading Paralax.MessageBrokers package to NuGet..."
